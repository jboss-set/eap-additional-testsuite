...
./Maven/apache-maven-3.5.3/bin/mvn clean install -Dquarkus -DJBOSS_VERSION=${JBOSS_VERSION} -Dquarkus-project-discovery=false -Dquarkus-cp-cache=false --debug -X -e
